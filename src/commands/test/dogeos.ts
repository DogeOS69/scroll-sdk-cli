...
const DEFAULT_HELPER_ABI: InterfaceAbi = [
  {
    inputs: [{ internalType: 'address payable', name: '_moat', type: 'address' }],
...
const DUST_LIMIT = 1_000_000n // 0.01 DOGE
const RBF_SEQUENCE = 0xff_ff_ff_fd
...
type ElectrsTxDetail = {
  ...
  txid: string
  vin: ElectrsTxVin[]
  vout: { scriptpubkey_address?: string; value: number | string }[]
}
...
${TEST_CASES.map((c) => `  - ${c.id}: ${c.name} - ${c.description}`).join('\n')}`
  ...
  static override flags = {
    ...
    'electrs-url': Flags.string({
      char: 'b',
      description: 'Electrs/Esplora API URL',
    }),
  ...
  public async loadConfig() {
    ...
    this.electrsURL = resolveElectrsUrl(flags['electrs-url'], this.networkName)
    ...
    this.log(`  Electrs URL: ${this.electrsURL || 'N/A'}`)
  ...
  private async broadcastRawTx(txHex: string): Promise<string> {
    ...
    if (this.l1RpcUrl && this.l1RpcUser && this.l1RpcPassword) {
      try {
        ...
        const reason = error instanceof Error ? error.message : String(error)
        this.warn(`RPC broadcast failed; falling back to Electrs. Reason: ${reason}`)
      }
    }

    const { result } = await broadcastTx(txHex, this.electrsURL)
    return result
  }
  ...
  ): Promise<{ finalFee: bigint; txid: string }> {
  ...
  private async buildAndBroadcastReplacementTx(
    ...
    const parentTxids = [...new Set(txDetail.vin.map((v) => v.txid))]
  ...
  private async buildAndBroadcastTx(keyPair: ECPairInterface, outputs: PsbtOutputParam[], verbose = false) {
    ...
    let utxos = await getUtxos(senderAddress, this.electrsURL)
    ...
    for (const utxo of utxos) {
      ...
      const txHex = (await getTx(utxo.txid, this.electrsURL)).hex
      psbt.addInput({
        hash: utxo.txid,
        index: utxo.vout,
    ...
    try {
      const { result: txid } = await broadcastTx(finalTxHex, this.electrsURL)
  ...
  private async caseBridgeUtxoAttack() {
    ...
    try {
      ...
      const outputValue = totalValue / (24n * BigInt(attackCount))
      const feeForDogeOs = 100_000_000n
      const feeForMiner = 1_000_000n

      const agentAddress = bitcoin.payments.p2pkh({ network: this.network, pubkey: agentKeyPair.publicKey }).address
      ...
      const masterConfirmed = await waitForConfirmations(txid, this.electrsURL, (msg) => this.log(msg), (msg) => this.warn(msg))
      if (!masterConfirmed) {
      ...
      const txHex = (await getTx(txid, this.electrsURL)).hex
      for (let i = 0; i < attackCount; i++) {
        const psbt = new bitcoin.Psbt({ network: this.network })
        psbt.addInput({
          hash: txid,
        ...
        const finalTxHex = psbt.extractTransaction().toHex()
        const { result: txid2 } = await broadcastTx(finalTxHex, this.electrsURL)
      ...
      while (retries < maxRetries) {
        try {
          ...
          if (currentBalance !== lastBalance) {
            const progress = Number((currentBalance * 10_000n) / requiredWei) / 100
    ...
    for (const entry of mempoolTxs) {
      ...
      try {
        ...
        for (const out of masterOutputs) {
          ...
          try {
            const { finalFee, txid: childTxid } = await this.buildAndBroadcastCpfpTx(
              txid,
              out.index,
              value,
              parentHex,
  ...
  private async caseFeeWalletAgentConsolidation() {
    ...
    try {
      ...
      const feeWalletUtxos = await getUtxos(this.feeWalletAddress, this.electrsURL)
      ...
      const fundingConfirmed = await waitForConfirmations(fundingTxid, this.electrsURL, (msg) => this.log(msg), (msg) => this.warn(msg))
      if (!fundingConfirmed) this.error(`Funding transaction ${fundingTxid} did not confirm within the expected time.`)

      const fundingTxHex = (await getTx(fundingTxid, this.electrsURL)).hex
      ...
      if (change >= DUST_LIMIT) {
        ...
        if (change >= DUST_LIMIT) {
          psbt.txOutputs.at(-1)!.value = change
        } else {
          includeChange = false
          psbt.txOutputs.pop()
      ...
      this.log(`   ${chalk.blue('Link:')} https://doge-testnet-explorer.qed.me/tx/${txid}`)
      await waitForConfirmations(txid, this.electrsURL, (msg) => this.log(msg), (msg) => this.warn(msg))
  ...
  private async caseLargePsbt() {
    ...
    const feePerInput = 1_000_000n // Generous fee for consolidation
    ...
    for (let i = 0; i < inputCount; i++) {
      ...
      const { address } = bitcoin.payments.p2pkh({ network: this.network, pubkey: keyPair.publicKey })
    ...
    this.log(chalk.gray('-> Waiting for funding confirmation...'))
    await waitForConfirmations(fundingTxid, this.electrsURL, (msg) => this.log(msg), (msg) => this.warn(msg))
    ...
    for (let i = 0; i < inputCount; i++) {
      try {
        ...
        this.error(`Failed to sign input ${i}: ${error instanceof Error ? error.message : String(error)}`)
      }
    ...
    const { result: largeTxid } = await broadcastTx(finalTxHex, this.electrsURL)
    ...
    await waitForConfirmations(largeTxid, this.electrsURL, (msg) => this.log(msg), (msg) => this.warn(msg))
  ...
  private async caseMultipleOutput() {
    ...
    try {
      ...
      const sanitizedOutputValue = outputValueRaw.replaceAll('_', '')
  ...
  private async caseMutipleWithdrawalPerTx() {
    ...
    try {
      ...
      const { deploymentData } = await this.foundryService.buildHelperDeploymentArtifacts(
        helperAbi,
        deploymentBytecode,
      ...
      const keyPair = ECPair.fromPrivateKey(new Uint8Array(randomBytes(32)), { network: this.network })
      const { address: withdrawalTargetDogeAddress } = bitcoin.payments.p2pkh({ network: this.network, pubkey: keyPair.publicKey })
  ...
  private async caseReplaceMempoolTxs() {
    ...
    for (const entry of mempoolTxs) {
      ...
      try {
        ...
        const originalFee =
          txDetail.fee === undefined ? 0n : BigInt(typeof txDetail.fee === 'string' ? txDetail.fee : txDetail.fee)
        const { finalFee, txid: replacementId } = await this.buildAndBroadcastReplacementTx(
          txDetail,
          keyPair,
          originalFee,
          prevHexCache,
  ...
  private async ensureHelperMoatConfigured(contract: Contract, moatAddress: string): Promise<void> {
    ...
    try {
      const contractWithSetter = contract as {
...
`)
  ...
  private getHelperContractConfig(): HelperContractConfig {
    const verification = {
      ...
      enabled: Boolean(this.l2ChainId),
    }
    ...
    const relativeCandidates = [
      { name: 'MultiWithdrawalHelper', segments: ['contracts', 'MultiWithdrawalHelper.sol'] },
    ]
  ...
  private async loadPrevTxHex(txid: string, cache: Map<string, string>): Promise<string> {
    ...
    const tx = await getTx(txid, this.electrsURL)
  ...
  private async prepareMutiWithdrawalCall(
    ...
    const fragment = contract.interface.getFunction('mutiWithdrawal')
    if (!fragment) {
  ...
  private async sendL2SelfTxWithRandomData(count = 3) {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const txRequest = { data: dataHex, to: wallet.address, value: 0 }
      const gasEstimate = await wallet.estimateGas(txRequest)
