...
export default class SetupDbInit extends Command {
  ...
  static override flags = {
    ...
    'no-ssl': Flags.boolean({
      description: 'Disable SSL for database connections (useful for local development)',
      default: false,
    }),
  }
  ...
  private async updatePermissions(conn: pg.Client, dbName: string, dbUser: string, debug: boolean, noSsl: boolean = false): Promise<void> {
    const queries = [
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      if (flags['update-permissions']) {
        ...
        this.conn = await this.createConnection(this.publicHost, this.publicPort, this.pgUser, this.pgPassword, this.pgDatabase, flags['no-ssl']);
      }
      ...
      for (const db of databases) {
        ...
        if (flags['update-permissions']) {
          ...
          await this.updatePermissions(this.conn, db.name, db.user.toLowerCase(), flags.debug, flags['no-ssl'])
        } else {
  ...
  private async createConnection(host: string, port: string, user: string, password: string, database: string, noSsl: boolean = false): Promise<pg.Client> {
    ...
    const conn = new pg.Client(connConfig)
