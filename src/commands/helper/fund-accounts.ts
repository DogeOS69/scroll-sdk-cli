...
export default class HelperFundAccounts extends Command {
  ...
  public async run(): Promise<void> {
    ...
    if (this.altGasTokenEnabled) {
      ...
      if (!this.l1GasTokenAddress || !this.l1GasTokenGateway) {
        this.error(
          'Alternative Gas Token is enabled but L1_GAS_TOKEN_ADDR or L1_GAS_TOKEN_GATEWAY_PROXY_ADDR is not set in config-contracts.toml',
        )
      }
    ...
    if (flags['fund-deployer']) {
      ...
      const l1Addresses: Record<string, string> = {
        ...
        L2_GAS_ORACLE_SENDER: config.accounts.L2_GAS_ORACLE_SENDER_ADDR, // The name is misleading, it's an account on l1
      }

      const l2Addresses: Record<string, string> = {
        L1_GAS_ORACLE_SENDER: config.accounts.L1_GAS_ORACLE_SENDER_ADDR, // The name is misleading, it's an account on l2
      }
  ...
  private async bridgeAltTokenL1ToL2(recipient: string, amount: number): Promise<void> {
    try {
      ...
      this.log(
        chalk.yellow(
          `Alternative gas tokens are being bridged to ${recipient}. Please wait for the transaction to be processed on L2.`,
        ),
      )
    } catch (error) {
      this.error(
  ...
  private async initializeAltGasToken(): Promise<void> {
    ...
    try {
      ...
      this.altGasTokenContract = new ethers.Contract(
        this.l1GasTokenAddress,
        erc20ABI,
        this.fundingWallet.connect(this.l1Provider),
      )
