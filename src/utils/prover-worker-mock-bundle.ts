...
import {
  MOCK_ADVANCE_L2_AGGREGATION_APP_COMMIT_RAW,
  MOCK_PROOF_IDENTITIES,
...
export interface ProverWorkerMockBundleOptions {
  /** Protocol L2 chain ID independently checked by the aggregation worker. */
...
const AGGREGATION_L2_CHAIN_ID_COMPOSE_ARGUMENT = `${String.fromCodePoint(36)}{AGGREGATION_L2_CHAIN_ID:?missing AGGREGATION_L2_CHAIN_ID}`
...
services:
  prover-worker:
    ...
    command:
      ...
      - ${MOCK_PROOF_IDENTITIES.advance_l2_aggregation.vkHash}
      - --aggregation-l2-chain-id
      - ${AGGREGATION_L2_CHAIN_ID_COMPOSE_ARGUMENT}
      - --enable-prove-bridge-transition
`

const PROVER_WORKER_MOCK_REQUIRED_AGGREGATION_ARGUMENTS = [
  '--aggregation-app-commit-raw-hex',
  MOCK_ADVANCE_L2_AGGREGATION_APP_COMMIT_RAW,
  '--aggregation-verification-key-hash',
  MOCK_PROOF_IDENTITIES.advance_l2_aggregation.vkHash,
...
export function verifyProverWorkerMockBundle(options: {
  ...
  for (const argument of PROVER_WORKER_MOCK_REQUIRED_AGGREGATION_ARGUMENTS) {
    if (!composeText.includes(argument)) {
      throw new Error(`${composePath}: required aggregation identity argument is missing: ${argument}`)
    }
  }

  const environment = readBundleEnvironment(envPath)
  requirePositiveInteger(
    environment.AGGREGATION_L2_CHAIN_ID || '',
    'AGGREGATION_L2_CHAIN_ID',
  )
...
export function hydrateProverWorkerMockBundle(options: {
  ...
  return writeProverWorkerMockBundle({
    aggregationL2ChainId: env.AGGREGATION_L2_CHAIN_ID || '',
    artifactReadBaseUrl,
    coordinatorUrl,
    dir: bundleDir,
