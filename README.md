...
* [`scrollsdk setup proof-release-init`](#scrollsdk-setup-proof-release-init)
...
USAGE
  $ scrollsdk setup doge-config [-c <value>] [--json] [-N] [--production-worker-launch external|local_cpu|local_cuda
    ...
    [--proof-release-lock <value> ] [--proof-resources-pvc <value> ] [--proof-topology-compiler-binary <value> ]
...
Import an immutable dogeos-core proof software release, bake deployment-bound Bridge material on CPU, and create the validated deployment lock

```
USAGE
  $ scrollsdk setup proof-release-init [--deployment-dir <value>] [--json] [-N] [--protocol-context <value>] [--release-image
    <value>] [-y]

FLAGS
  -N, --non-interactive           Run without prompts; --release-image is required for a new import
  -y, --yes                       Pull, validate, and bake without the final confirmation
      --deployment-dir=<value>    Deployment root containing .data/protocol_context.json
      --json                      Output structured JSON
...
DESCRIPTION
  Import an immutable dogeos-core proof software release, bake deployment-bound Bridge material on CPU, and create the
  validated deployment lock

EXAMPLES
  $ scrollsdk setup proof-release-init

  $ scrollsdk setup proof-release-init --release-image dogeos69/proof-release@sha256:<digest>

  $ scrollsdk setup proof-release-init --release-image dogeos69/proof-release@sha256:<digest> -N
```
